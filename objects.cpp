...
class snake
{
    ...
    public:
        ...
        void move_body(struct t_view* vi)
        {
            vi->draw(body[0].first, body[0].second, ' ');
        ...
        void draw_body(struct t_view* vi)
        {
            for (int i = 0; i <= body.size(); i++)
                vi->draw(body[i].first, body[i].second, '*');
        }
...
class border
{
    ...
    public:
        ...
        void draw_border(struct t_view* vi)
        {
...
class food
{
    ...
    public:
        ...
        void spawn(class snake* sn)
        {
            while (1)
            {
                int rand_x = rand() % (x_len - 1) + 1;
                int rand_y = rand() % (y_len - 1) + 1;
        ...
        void draw_food(struct t_view* vi)
        {
            vi->draw(x, y, '0');
                printf("\e[%d;%d;H", 22, 0);
                printf("%d %d", x, y);
        ...
        void del_food(struct t_view* vi)
        {
            vi->draw(x, y, ' ');
        }
